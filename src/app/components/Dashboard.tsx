...
import { MoreHorizontal, Check, Pill, Map } from "lucide-react";
...
export function Dashboard({ onNavigate }: DashboardProps) {
  ...
  function handleUncheckStep(index: number) {
    // Remove this step and all subsequent completed steps
    const newCompleted = stepState.completedIndices.filter((i) => i < index);
    const updated = {
      ...stepsProgress,
      [visitId]: { currentIndex: index, completedIndices: newCompleted },
    };
    setStepsProgress(updated);
    localStorage.setItem(NEXT_STEPS_KEY, JSON.stringify(updated));
  }
  ...
  return (
    <div className="flex flex-col min-h-screen pb-28 px-5 pt-14">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-[#1e2533]" style={{ fontSize: 13 }}>{dateDisplay}</p>
        ...
        >
          <span className="text-white" style={{ fontSize: 14, fontWeight: 700 }}>K</span>
        </button>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {checkedInToday ? (
          <div
            className="bg-[#eaedf4] rounded-[20px] px-[14px] py-[18px] flex flex-col gap-[10px]"
          >
            <div className="flex flex-col gap-1">
              <p className="text-[#1e2533]" style={{ fontSize: 17, fontWeight: 700 }}>
            ...
            <button
              ...
              className="rounded-full px-[10px] py-[6px] self-start border border-[#1e2533]"
              style={{ fontSize: 12, fontWeight: 600, color: "#1e2533" }}
            >
              Log again
            </button>
          </div>
          ...
          <div
            className="bg-[#577399] rounded-[20px] px-[14px] py-[18px] flex flex-col items-center gap-4 text-center"
          ...
          >
            ...
            <button
              onClick={() => onNavigate("/checkin")}
              className="bg-[#f5f7fa] text-[#577399] rounded-full px-5 py-2.5"
        ...
        {lastVisit && nextSteps.length > 0 && (
          <div
            className="bg-white rounded-[20px] p-[18px] flex flex-col gap-[14px]"
          ...
          >
            ...
            <div className="flex flex-col gap-0.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Map className="w-4 h-4 text-[#1e2533]" strokeWidth={2} />
            ...
            {/* Collapsed: show current step only */}
            {!showAllSteps && (
              <button
                onClick={!allDone ? handleCheckStep : undefined}
                className="flex items-start gap-[12px] w-full text-left"
                disabled={justChecked || allDone}
              >
                {/* Number + checkbox stack */}
                <div className="flex flex-col items-center gap-1 shrink-0">
                  <p className="text-[#a9b9d0]" style={{ fontSize: 10, fontWeight: 600 }}>
                    {allDone ? nextSteps.length : currentIndex + 1}/{nextSteps.length}
                  </p>
                  <div
                    className={`w-[26px] h-[26px] rounded-[6px] flex items-center justify-center transition-colors ${
                      allDone || justChecked ? "bg-[#465e83]" : "border border-[#1e2533]"
                    }`}
                  >
                    {(allDone || justChecked) && (
                ...
                <div className="flex flex-col gap-0.5 flex-1">
                  <p
                    className={`transition-colors ${allDone || justChecked ? "text-[#7a94b6] line-through" : "text-[#1e2533]"}`}
                    style={{ fontSize: 15, fontWeight: 600, lineHeight: "1.35" }}
                  >
                    {currentStep?.title}
                  </p>
                  <p
                    className={`transition-colors ${allDone || justChecked ? "text-[#a9b9d0]" : "text-[#7a94b6]"}`}
            ...
            {showAllSteps && (
              <div className="flex flex-col gap-3">
                {nextSteps.map((step, i) => {
                  ...
                  const isAnimating = isCurrent && justChecked;
                  const isDone = isCompleted || isAnimating;
                  ...
                  return (
                    <button
                      key={i}
                      onClick={
                        isCompleted ? () => handleUncheckStep(i)
                        : isCurrent && !justChecked ? handleCheckStep
                        : undefined
                      }
                      disabled={isLocked || justChecked}
                      className="flex items-start gap-[12px] w-full text-left"
                    >
                      <div className="flex flex-col items-center gap-1 shrink-0">
                        <p className="text-[#a9b9d0]" style={{ fontSize: 10, fontWeight: 600 }}>
                          {i + 1}/{nextSteps.length}
                        </p>
                        <div
                          className={`w-[26px] h-[26px] rounded-[6px] flex items-center justify-center transition-colors ${
                            isDone
                      ...
                      <div className="flex flex-col gap-0.5 flex-1">
                        <p
                          className={`${isDone ? "text-[#7a94b6] line-through" : isLocked ? "text-[#a9b9d0]" : "text-[#1e2533]"}`}
                          style={{ fontSize: 15, fontWeight: 600, lineHeight: "1.35" }}
                        >
                          {step.title}
                        </p>
                        <p
                          className={`${isDone ? "text-[#a9b9d0]" : isLocked ? "text-[#d1d9e6]" : "text-[#7a94b6]"}`}
                          style={{ fontSize: 13, lineHeight: "1.5" }}
                        >
                          {step.description}
                        </p>
        ...
        {lastVisit && medSchedule.length > 0 && (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Pill className="w-4 h-4 text-[#1e2533]" strokeWidth={3} />
                <p className="text-[#1e2533]" style={{ fontSize: 16, fontWeight: 600 }}>
                  Today's Medications
                </p>
              </div>
        ...
        {!lastVisit && (
          <div
            className="bg-white rounded-[20px] p-[18px] flex flex-col items-center gap-4 text-center"
            style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.07)" }}
          >
            <p className="text-[#1e2533]" style={{ fontSize: 14 }}>
              Record a visit to see your next steps and medications here.
            </p>
            <button
              onClick={() => onNavigate("/summary")}
              className="bg-[#577399] text-white rounded-full px-5 py-2.5"
              style={{ fontSize: 14, fontWeight: 600 }}
            >
              Log visit
            </button>
    ...
    >
      <div className="flex items-center gap-[10px]">
        ...
        <div className="flex flex-col">
          <span
            className={med.taken ? "text-[#7a94b6] line-through" : "text-[#1e2533]"}
          ...
          <div className="flex items-center gap-1">
            {urgent && (
              <span className="w-1.5 h-1.5 rounded-full bg-[#577399] shrink-0" />
            )}
            <span style={{ fontSize: 10, lineHeight: "1.5", color: urgent ? "#577399" : undefined, opacity: urgent ? 1 : 0.7 }}>
              {urgent ? `Missed · ${med.time}` : med.time}
            </span>
          </div>
        </div>
      </div>
      <button onClick={onEdit}>
        <MoreHorizontal className="w-5 h-5 text-[#a9b9d0]" />
      </button>
    </div>
  );
}
